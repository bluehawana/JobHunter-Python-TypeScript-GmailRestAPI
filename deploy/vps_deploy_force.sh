#!/bin/bash
# Run this script ON the VPS server - handles conflicts by stashing local changes
# Usage: ssh root@jobs.bluehawana.com 'bash -s' < deploy/vps_deploy_force.sh

set -e

echo "🚀 VPS Deployment with Atomic Restart (Force)"
echo "=============================================="
echo ""

# Step 1: Pull latest code (handle conflicts)
...
git stash push -m "Auto-backup before deploy $(date '+%Y-%m-%d %H:%M:%S')" 2>/dev/null || true
...
echo ""
echo "📝 Note: Local changes were stashed. To view:"
echo "   cd /var/www/lego-job-generator && git stash list"
